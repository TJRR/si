...
<h1><?php echo $slide === null ? 'Novo quadro: ' : 'Editar quadro: '; ?><?php echo htmlspecialchars($evento['nome'], ENT_QUOTES, 'UTF-8'); ?></h1>

<?php if (!empty($erro)): ?>
    <p style="color:red;"><?php echo htmlspecialchars($erro, ENT_QUOTES, 'UTF-8'); ?></p>
<?php endif; ?>

<form method="post" action="<?php echo $slide === null ? url('eventoSlides/novo/' . (int) $evento['id']) : url('eventoSlides/editar/' . (int) $slide['id']); ?>" enctype="multipart/form-data"><?= campoCsrf() ?>
    <fieldset>
    ...
    <fieldset>
        <legend>Etiqueta acima do título (opcional)</legend>
        <p style="color:#555;font-size:0.9em;">Selo curto, em caixa alta, acima do título do quadro. Exemplo: "PRAZO ATÉ 16/10".</p>

        <label>Texto da etiqueta:
            <input type="text" name="etiqueta_texto" maxlength="60" value="<?php echo htmlspecialchars($slide !== null ? (string) $slide['etiqueta_texto'] : '', ENT_QUOTES, 'UTF-8'); ?>">
        </label><br>

        <?php $nome = 'etiqueta_cor_fundo'; $valor = $slide !== null ? $slide['etiqueta_cor_fundo'] : null; $rotulo = 'Cor de fundo da etiqueta'; $padrao = '#141413'; ?>
        <?php include __DIR__ . '/../_campo_cor.php'; ?>

        <?php $nome = 'etiqueta_cor_texto'; $valor = $slide !== null ? $slide['etiqueta_cor_texto'] : null; $rotulo = 'Cor do texto da etiqueta'; $padrao = '#FFFFFF'; ?>
        <?php include __DIR__ . '/../_campo_cor.php'; ?>
    </fieldset>
    ...
    <fieldset>
        ...
        <hr>
        <p style="color:#555;font-size:0.9em;">Segundo botão (opcional), desenhado com contorno, ao lado do primeiro.</p>

        <label>Título do segundo botão:
            <input type="text" name="cta2_titulo" value="<?php echo htmlspecialchars($slide !== null ? (string) $slide['cta2_titulo'] : '', ENT_QUOTES, 'UTF-8'); ?>">
        </label><br>

        <label>Hiperlink do segundo botão:
            <input type="text" name="cta2_link" value="<?php echo htmlspecialchars($slide !== null ? (string) $slide['cta2_link'] : '', ENT_QUOTES, 'UTF-8'); ?>">
        </label><br>

        <label>Abrir o segundo botão em:
            <select name="cta2_target">
                <option value="_self" <?php echo ($slide !== null && $slide['cta2_target'] === '_self') ? 'selected' : ''; ?>>Mesma aba</option>
                <option value="_blank" <?php echo ($slide !== null && $slide['cta2_target'] === '_blank') ? 'selected' : ''; ?>>Nova aba</option>
            </select>
        </label><br>

        <?php $nome = 'cta2_cor_fundo'; $valor = $slide !== null ? $slide['cta2_cor_fundo'] : null; $rotulo = 'Cor de fundo do segundo botão'; $padrao = '#FFFFFF'; ?>
        <?php include __DIR__ . '/../_campo_cor.php'; ?>

        <?php $nome = 'cta2_cor_texto'; $valor = $slide !== null ? $slide['cta2_cor_texto'] : null; $rotulo = 'Cor do texto do segundo botão'; $padrao = '#141413'; ?>
        <?php include __DIR__ . '/../_campo_cor.php'; ?>
    </fieldset>
    ...
    <div class="form-acoes">
        <a href="<?php echo url('eventoSlides/index/' . (int) $evento['id']); ?>" class="btn-voltar">Voltar</a>
