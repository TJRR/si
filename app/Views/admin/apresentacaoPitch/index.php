...
<form method="post" action="<?php echo url('apresentacaoPitchAdmin/salvarConfig'); ?>"><?= campoCsrf() ?>
    ...
    <label>Janela para as equipes escolherem data/horário — início:
    ...
    <label>E-mail organizador do Google Agenda (conta institucional @tjrr.jus.br — os eventos das apresentações online são criados nesta conta):
    ...
    <div class="tabela-scroll">
        <table>
            ...
            <tr>
                ...
                <td><?php echo $slot['modalidade'] !== null ? ($slot['modalidade'] === 'online' ? 'Online' : 'Presencial') : '—'; ?></td>
                <td>
        ...
        <label>Horário:
            <select name="id" required>
                <?php foreach ($slotsVagos as $slot): ?>
                    <option value="<?php echo (int) $slot['id']; ?>"><?php echo htmlspecialchars(formatarDataHora($slot['data_inicio']) . ' — ' . formatarDataHora($slot['data_fim']), ENT_QUOTES, 'UTF-8'); ?></option>
                <?php endforeach; ?>
            </select>
        </label>
